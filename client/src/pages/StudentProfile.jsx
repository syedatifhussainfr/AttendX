...
function Calendar({ records }) {
  const latest =
    records[0]?.session.date || new Date().toISOString().slice(0, 10);
  const [cursor, setCursor] = useState(
    () => new Date(`${latest.slice(0, 7)}-01T00:00:00`),
  );
  ...
  return (
    <section className="panel student-calendar-panel">
      <header>
        <div>
          <span className="eyebrow">ATTENDANCE CALENDAR</span>
          <h2>{monthName.format(cursor)}</h2>
        </div>
        <div>
      ...
      <div className="student-calendar-grid">
        {cells.map((day, index) => {
          ...
          return (
            <span
              className={entries.length ? "has-records" : ""}
              key={key}
              title={entries
                .map((row) => `${row.session.subject.code}: ${row.status}`)
                .join(" · ")}
            >
              <b>{day}</b>
              <i>
                {entries.slice(0, 4).map((row) => (
                  <em className={row.status.toLowerCase()} key={row.id} />
                ))}
              </i>
            </span>
          );
...
export function StudentProfile() {
  ...
  const download = async () => {
    ...
    try {
      ...
      toast(`Student report downloaded as ${filename}.`);
    } catch (error) {
    ...
    <div className="page student-profile-page">
      <div className="student-profile-actions">
        ...
        <button className="secondary" onClick={download} disabled={downloading}>
          <Download /> {downloading ? "Preparing…" : "Download report"}
        </button>
      </div>
      <section className="student-profile-hero">
        ...
        <div className="student-profile-title">
          <span className="eyebrow">ROLL {student.rollNumber}</span>
          <h1>{student.name}</h1>
          <p>
            {[
        ...
        <div
          className={`student-profile-standing ${attendance.risk.toLowerCase()}`}
        >
          <span>{riskLabel[attendance.risk]}</span>
          <strong>
            {attendance.percentage == null
              ? "—"
              : `${attendance.percentage.toFixed(2)}%`}
          </strong>
          <small>Target {attendance.target}%</small>
        </div>
      </section>
      <section className="student-profile-metrics">
        <article>
          ...
          <small>{attendance.appearance} physical appearances</small>
        </article>
        <article className="present">
          <span>Present</span>
          <strong>{attendance.present}</strong>
          <small>Credited attendance</small>
        </article>
        <article className="late">
          <span>Late</span>
          <strong>{attendance.late}</strong>
          <small>Credit follows each session rule</small>
        </article>
        <article className="absent">
          ...
          <small>
            {attendance.absenceStreak
              ? `${attendance.absenceStreak} current streak`
              : "No current streak"}
          </small>
        </article>
      </section>
      <section className="student-guidance-panel">
        <div>
          ...
          <strong>
            {attendance.percentage == null
              ? "No closed attendance records yet"
              : attendance.risk === "GOOD"
                ? `Can miss ${attendance.classesCanMiss} more class${attendance.classesCanMiss === 1 ? "" : "es"} and remain at target`
                : attendance.classesNeeded == null
                  ? "A 100% target requires every future class to be Present"
                  : `Attend ${attendance.classesNeeded} consecutive class${attendance.classesNeeded === 1 ? "" : "es"} to reach ${attendance.target}%`}
          </strong>
        </div>
      ...
      <div className="student-profile-layout">
        <section className="panel student-subject-panel">
          <header>
            ...
            <small>{subjects.length} subjects with records</small>
          </header>
            ...
            {subjects.map((subject) => (
              <div className="row" key={subject.id}>
                ...
                <span>
                  ...
                  <i className={`student-risk ${subject.risk.toLowerCase()}`}>
                    {riskLabel[subject.risk]}
                  </i>
                </span>
              </div>
            ))}
        ...
        <section className="panel student-info-panel">
          ...
          <dl>
            ...
            <div>
              <dt>Enrolment</dt>
              <dd>{student.enrollmentNumber || "Not provided"}</dd>
            </div>
            <div>
              <dt>Section</dt>
              <dd>{student.section || "Not provided"}</dd>
            </div>
            <div>
              <dt>Admission date</dt>
              <dd>
                {student.admissionDate
                  ? shortDate(student.admissionDate)
                  : "Not provided"}
              </dd>
            </div>
            {"phoneNumber" in student && (
              <>
                <div>
                  ...
                  <dd>{student.phoneNumber || "Not provided"}</dd>
                </div>
                <div>
                  <dt>
                    <Phone />
                  ...
                  <dd>{student.guardianPhone || "Not provided"}</dd>
                </div>
                <div className="notes">
                  <dt>
                    <MailWarning />
                  ...
                  <dd>{student.notes || "No administrative notes"}</dd>
                </div>
              </>
            )}
          </dl>
      ...
      <div className="student-profile-lower">
        ...
        <section className="panel student-timeline-panel">
            ...
            {records.slice(0, 12).map((record) => (
              <article key={record.id}>
                ...
                <div>
                  ...
                  <small>
                    {shortDate(record.session.date)} ·{" "}
                    {record.session.startTime}–{record.session.endTime}
                  </small>
                  {record.correctedAt && (
                    <em>Corrected · {record.correctionReason}</em>
                  )}
                </div>
                <b>{record.status}</b>
              </article>
          ...
          {!records.length && (
            <p className="empty-copy">
              No closed attendance records are available.
            </p>
          )}
        </section>
      </div>
